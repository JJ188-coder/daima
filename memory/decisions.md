...
### [2026-07-08] 6 列 HUICE_COLS + 亏损标红 + 运营看板

**背景**: 原 4 列(净利润/净利率/推广费比/保本ROI)不够用,运营需要更多指标看盈亏全貌

**决策**:
1. **HUICE_COLS 从 4 列扩到 6 列** -- 新增「毛利率」+「退款额」,放在净利润/净利率后面
2. **净利润列亏损标红** -- `huice-netProfit < 0` 时用红色 `#f5222d` + 加粗显示,一眼看出亏损商品
3. **推广费比优化** -- 如果 mms 有 spend 但无 gmv,用 `huice.salesAmount` 估算 GMV 算推广费比
4. **运营看板** -- `tools/huice-report.mjs` 生成报告:总览/亏损 TOP 10/盈利 TOP 10/店铺排名

**理由**:
- 毛利率 + 退款额 = 完整的盈亏诊断(光看净利率分不清是毛利低还是推广费高)
- 亏损标红 = 运营扫一眼就知道哪些商品在亏钱
- 推广费比用 salesAmount 估算 = mms 无 GMV 时也有值,不至于全 null

**影响文件**:
- `dts/source/pdd-enhancer.js` -- HUICE_COLS 6 列 + renderCell 亏损标红 + 推广费比优化
- `tools/huice-report.mjs` -- 运营看板报告工具

**验证**: 语法检查通过 `node --check dts/source/pdd-enhancer.js`

**撤销条件**: 运营不需要毛利率/退款额,或列太多影响表格性能

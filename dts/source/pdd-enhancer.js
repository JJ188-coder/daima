...
async function getPromoDataByWindow(window) {
  ...
  async function fetchShopProfitRange(start, end, mallId) {
    if (!mallId) return { status: 'no_mapping', mapping: null, days: [] };
  ...
  function renderShopProfitPanel(panel, data, start, end, mallId) {
    ...
    if (validDays.length > 0) {
      ...
      html += '<th style="padding:4px 8px;border:1px solid #722ed1;text-align:right;">保本ROI</th>';
      html += '<th style="padding:4px 8px;border:1px solid #722ed1;text-align:right;">费比</th>';
